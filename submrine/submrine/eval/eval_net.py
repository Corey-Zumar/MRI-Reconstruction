...
def get_oasis_image_paths(data_path):
    oasis_subdirs = [
        subdir for subdir in os.listdir(data_path)
        if OASIS_DATA_DIRECTORY_PREFIX in subdir
    ]
    oasis_raw_paths = []
    for subdir in oasis_subdirs:
        raws_subdir = os.path.join(data_path, subdir,
                                   OASIS_DATA_RAW_RELATIVE_PATH)
        for raw_fname in [
                fname for fname in os.listdir(raws_subdir)
                if OASIS_DATA_EXTENSION_IMG in fname
        ]:
...
def get_prostate_image_paths(data_path):
    ps_subpaths = [
        path for path in os.listdir(data_path)
...
def load_image(image_path, substep):
    ...
    original_data = np.array(
        np.squeeze(original_img.get_data()), dtype=np.float32)
    ...
    subsampled_img, subsampled_K = subsample(
        image_path, substep=substep, low_freq_percent=LOW_FREQ_PERCENT)
...
def eval_diff_plot(net_path, img_path, substep):
    test_subsampled, test_subsampled_K, test_original = load_image(
        img_path, substep)
    ...
    original_img = normalize_data(test_original[5][63:319, 63:319].reshape(
        256, 256))
