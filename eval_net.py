...
def load_image(image_path):
    ...
    original_data = np.moveaxis(original_data, -1, 0).reshap(128, 256, 256, 1)
...
def main():
    ...
    result = fnet.predict(test_subsampled[0].reshape(1, 256, 256, 1))

    magnitude_spectrum = 20*np.log(np.abs(result.reshape(256,256)))
    plt.subplot(121),plt.imshow(test_original, cmap = 'gray')
